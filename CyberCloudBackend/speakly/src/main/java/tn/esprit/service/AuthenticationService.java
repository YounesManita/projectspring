...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        // 1. Valider les credentials et récupérer l'utilisateur
        ...
        // 4. Gérer les tokens en base
        tokenService.revokeAllUserTokens(user);
        tokenService.saveUserToken(user, accessToken);

        // 5. Retourner la réponse avec l'ID utilisateur
        return new AuthenticationResponse(
                accessToken,
                refreshToken,
                user.getId()  // Ajout de l'ID utilisateur
        );
    }
            ...
            try {
                ...
                if (username != null) {
                    ...
                    if (jwtService.isTokenValid(refreshToken, userDetails)) {
                        ...
                        // set the result
                        result = new AuthenticationResponse(accessToken, refreshToken, result.user_id());
                    }
                }
